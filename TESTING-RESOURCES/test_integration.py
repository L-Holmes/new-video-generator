...
Lives in TESTING-RESOURCES/ — it adds the repo root to sys.path itself.
"""
import sys, types, shutil
from pathlib import Path

HERE = Path(__file__).resolve().parent
REPO = HERE.parent                       # TESTING-RESOURCES/ sits in the root
for p in (HERE, REPO):
...
dead = {"joint_3_row", "stickman_joint_3_row", "decorate_previous",
        ...
        "read_out", "stickman", "edit", "object_generate",
...
_sg_src = (REPO / "___visuals" / "scene_generators.py").read_text()
...
_dl = _il.import_module("___visuals.sources.downloads")
...
import importlib
_sg_file = REPO / "___visuals" / "ai" / "generate_stickman_images.py"
sg = importlib.import_module("___visuals.ai.generate_stickman_images") \
     if _sg_file.exists() else None
if sg:

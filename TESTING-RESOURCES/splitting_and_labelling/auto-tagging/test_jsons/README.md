...
Driven entirely by `TEST_AUTO_TAGGER.py` in the directory above:
`uv run TESTING-RESOURCES/splitting_and_labelling/auto-tagging/TEST_AUTO_TAGGER.py`.
...
`*-CACHE` folders and `uv run TESTING-RESOURCES/splitting_and_labelling/auto-tagging/TEST_AUTO_TAGGER.py --build` puts them back
(it re-runs the splitter, which takes a minute).

The ten are deliberately different kinds of video, not ten explainers — see
`uv run TESTING-RESOURCES/splitting_and_labelling/auto-tagging/TEST_AUTO_TAGGER.py --list` for what each one
is there to probe.

...
1) On the manual tagger, could we extend the keyword recommendation? 



Keep a track o fpreviously identified visualisables. 
...
when i look through the eye of the needle, am i looking at the side of the presser foot? 

etc.

Please add your own as well- but in code arrange in a really obvious fashion, idiot proof. well commented etc. 

Then! For the current entry, we look at the extracted visualisables, and then look at the previous visualisables and then use some logic / existing library (e.g. like an auto correct or translation thing or just something that knows if words pair together naturally?) to know if they are likely to be linked. If so, we then present that as a tab option / recommended word, alongside the other visualisables that identified for that sentence.



So basically create scores / probabilities on [most] key words, and then if/how well they fit together with current context.

In fact, then in another stage, use the same logic to see if we can identify what abstract words relate to...

e.g. things like 'they', 'he', 'it' etc. 

for that, on the manual tagger, add a small scrollable section with a few of the probabilities/scores we have for that.

like: 

- 'he' -> Jerry Patternly (0.6)

- 'it' -> sausage roll (0.4)

- 'it' -> car (0.7)

- 'it' (2) -> sausepan (0.2)   [the (2) here shows its the second 'it' in the sentence]
...
--> We would see 'Jar' in the current, and then 'nutmeg' in the previous. We'd think 'would "jar nutmeg" make sense? yes! (jar concrete? no!); so we'd recommend "Jar of Nutmeg"


(by the way, if you feel the above is complex enough to be in its own file... please do that and import in the manual tagger...)


2) On manual tagger, sometimes when tabbing through, if I'm just decorating the previous for instance, I don't want to accept the suggestion... do I change it to like right arrow or something? 



3) On the manual tagger, if im decorating previous by stamping stock, and thus step 3 comes up... after selecting an option and i tab it moves to the next step 3 option. It should auto go to continue to next, and continue to the next one, much like how it works if its just step 2... 

...
# Allow running this file directly from the repo root (uv run ___visuals/stock_footage_review.py).
...
class _MediaReviewer:
    ...
    def _on_try_again(self):
        ...
        Only meaningful for AI scenes (stickman / edit / stickman_joint); the
        ...
        if self._regenerate_fn is None or (
            ...
            self.status_var.set(
                "Try-again only works for AI scenes (stickman / edit)."
            )
            return
    ...
    def _on_search_again(self, widen: bool):
        ...
        def worker():
            result, err = None, None
            try:
                from ___visuals.sources.downloads import refetch_candidates
...
def run_media_review(
    ...
    regenerate_fn : callable | None
        ...
        edit) — the caller owns deleting stale outputs and registering the

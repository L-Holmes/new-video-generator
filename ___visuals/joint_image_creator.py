...
Features
--------
- Random background pick from `.resources/backgrounds/` (or use a specific path).
...
* ...
* Gets a random background from BACKGROUNDS_DIR = .resources/backgrounds/ (unless its given a specific filename, in which case it will just get that specific one)
...
from CONFIG import BACKGROUNDS_DIR as _CFG_BACKGROUNDS_DIR
from moviepy import (
    CompositeVideoClip,
    ImageClip,
    VideoFileClip,
...
BACKGROUNDS_DIR: str = str(_CFG_BACKGROUNDS_DIR)
...
def _ensure_test_assets() -> List[str]:
    ...
    overlay_dir = str(Path(__file__).resolve().parent.parent
                      / "TESTING-RESOURCES" / "images")
...
if __name__ == "__main__":
    ...
    if not any(Path(BACKGROUNDS_DIR).iterdir()):
        shutil.copy(IMG1, str(Path(BACKGROUNDS_DIR) / "_auto_bg.jpg"))

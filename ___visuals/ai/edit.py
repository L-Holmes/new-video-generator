...
uv run edit.py temp/ai_output/005.png "raise the sword hand" -o temp/ai_output/005-hand-raised.png
"""
...
For scenes flagged search_type == "edit", takes a BASE image (the chosen
image of the nearest preceding AI scene - stickman or another edit) and
...
Base resolution (done in main.py, passed in here as `ordered_scenes`):
  - Walk backwards from the edit scene to the nearest preceding scene that is
    an AI base (stickman or edit). Non-AI scenes (stock/wikipedia/joint/
...
placeholder) from generate_stickman_images so edits get the same out-of-
...
import ___visuals.ai.generate_stickman_images as sg
...
def resolve_edit_bases(ordered_scenes, out_dir):
    ...
    ordered_scenes : list of dicts in SCRIPT ORDER, each:
        {
          "script_text":  str,
          "is_edit":      bool,           # this scene is edit
          "is_ai_base":   bool,           # eligible as an edit base (stickman or edit)
...
async def _edit_one(sem, script_text, instruction, base_image_path, ref_urls,
    ...
    if base_image_path is not None and use_fresh:
        print(f"[edit] base missing on disk for '{script_text[:45]}' "
...
async def _generate_all(ordered_scenes, out_dir, num_variants):
    ...
    print(f"Processing {len(edit_scenes)} edit scene(s) x {num_variants} variant(s)")
    if not edit_scenes:
        return {}
    ...
    async def _upload_ctx(path):
        ...
        if not pathlib.Path(path).exists():
            print(f"[edit] context image missing on disk, skipping: {path}")
            return None
    ...
    for scene in edit_scenes:
        ...
        if context_by_text.get(text):
            print(f"[edit] '{text[:45]}' using {len(context_urls)} "
                  f"context image(s)")
    ...
    if placeholder_scenes:
        print("\n" + "!" * 70)
        print(f"[edit] WARNING: {len(placeholder_scenes)} edit scene(s) got a "

...
* caption_text - optional, add by hand: the caption to draw when a line stacks the caption modifier on a base that NEEDS its search_term for fetching (e.g. stock + caption). when absent, the caption text is the search_term (same as the old overlay type).
